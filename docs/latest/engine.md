...
四个槽位带固定提示语(`server/mockLLM.js:268`):**A「稳妥但费工」/ B「程序优先」/ C「经营关系」/ D「省事但有代价」**。效果数值绑定场景与槽位语义供给,因此槽位符号模式可以全库扫描:
...
mock 的衔接语也只回引 prompt 中真实存在的上一步标题(`:262`):`承接「上一步标题」的余波,事情还没完。`——不捏造人名(第一轮审计曾抓到 v1 mock 凭空捏造人名)。
...
2. **easy 难度属性顶格**:good/mixed 玩家中盘起四维常全满 100(500 人中 219 人),clamp 边界大量出现(第 9 节);mock 效果多为正和是放大因素,真实 GLM 分布更分散。
